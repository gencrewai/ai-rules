...
### 4. 멀티 도구 동기화 (단일 소스 → 10+ 러너)

하나의 규칙 소스에서 **주요 AI 코딩 러너 전부**에 대한 출력을 각 도구의
네이티브 포맷으로 생성합니다. 내부 참조(`CLAUDE.md`, `.claude/agents/...`)는
각 도구 경로로 자동 치환됩니다.

```
core/rules/      ──┬──→  CLAUDE.md                          (Claude Code — 네이티브 서브에이전트)
core/agents/     ──┼──→  AGENTS.md + .codex/agents/         (Codex — 네이티브 서브에이전트)
                   ├──→  .cursor/rules/ + .cursor/rules/agents/*.mdc   (Cursor — 역할 규칙)
                   ...
                   ├──→  .kilo/ / .augment/ / .trae/  (generic 어댑터 — YAML만으로 추가)
                   └──→  AI-RULES.md                                   (도구 중립 fallback)
```

- **네이티브 서브에이전트 지원 러너** (Claude Code, Codex): frontmatter + body를
  그대로 복사
- **역할-규칙 변환 러너** (Cursor, Windsurf, Gemini, Copilot, Cline, Antigravity):
  각 에이전트를 use-case 범위의 역할 프롬프트로 변환 + 경로 치환 +
  Claude 전용 지시("Task 툴로 호출하라" 등) 중립화
- **롱테일 러너** (Kilo / Augment / Trae / 향후 신규 도구): YAML 한 블록으로
  추가 — 어댑터 코드 변경 불필요 (`generic` 어댑터)

Sync는 **manifest로 추적**됩니다: 도구를 on/off할 때마다 이전 배포 파일과
비교해 정리하고, 사용자가 로컬에서 직접 수정한 파일은 기본적으로
덮어쓰지 않으며, 깔끔한 uninstall을 지원합니다.
...
- 9개 에이전트 역할 정의를 **선택한 도구(기본 Claude Code)에 배포**
- 스택별 `.env.example`과 도구 권한 생성
- Git 초기화 + develop 브랜치 자동 생성 (main 보호)
- AI 로그 및 docs 디렉토리 구조 설정

기본은 외부 의존성 없음 (Node.js 내장 모듈만, npm install 불필요).

**멀티 도구로 에이전트 배포** — `--tools` 플래그:

```bash
# Claude Code + Codex CLI + Cursor 에이전트를 함께 배포
node engine/cli/scaffold.mjs --name my-app --tools claude-code,codex,cursor
```

| 도구 | 출력 위치 | npm install 필요? |
|---|---|---|
| `claude-code` (기본) | `.claude/agents/*.md` (원본 그대로) | 불필요 |
| `codex` | `.codex/agents/*.md` (경로 치환) | 불필요 |
| `cursor` | `.cursor/rules/agents/*.mdc` (MDC 변환 + Claude 지시문 중립화) | 필요 (`js-yaml`) |

> scaffold의 `--tools`는 **에이전트 파일만** 멀티 도구 배포합니다. 규칙 파일
> 자체(`AGENTS.md` for Codex, `.cursor/rules/*.mdc` for Cursor)와 나머지 러너
> (Gemini / Windsurf / Copilot / Cline / Antigravity / 롱테일) 지원은
> 아래 [방법 D: sync 엔진](#방법-d-sync-엔진-멀티-프로젝트--멀티-도구)에서 제공합니다.
...
# 2. 에이전트 역할 정의 복사 (Claude Code)
cp core/agents/*.md your-project/.claude/agents/
```

Codex / Cursor / Windsurf / Gemini / Copilot / Cline 등 다른 러너용 레이아웃은
[core/README.md](core/README.md)의 도구별 네이티브 경로 표를 참고하거나,
sync 엔진에 맡겨 자동 변환하세요.

### 방법 D: sync 엔진 (멀티 프로젝트 + 멀티 도구)

동일한 규칙을 여러 프로젝트에, 그리고 프로젝트마다 여러 AI 도구에
동시에 배포하려면 sync 엔진을 사용합니다:

```bash
cd ai-rules
npm install
npm run new -- my-project       # 프로필 생성
npm run sync                    # output/ 에 생성 (미리보기)
npm run sync:apply              # 타깃 프로젝트에 복사
npm run sync:prune              # 적용 + 비활성화된 도구 흔적 제거
npm run sync:uninstall -- --project my-project   # 전부 제거
```

매 실행마다 manifest(`output/<project>/sync-status.json`)가 파일 경로 +
내용 해시를 추적합니다. 그래서 도구를 나중에 끄면 해당 파일들이 자동
정리되고, 사용자가 로컬에서 직접 수정한 파일은 덮어쓰기로부터 보호됩니다.
...
| 멀티 도구 | 단일 소스 → 10+ 러너 (Claude / Codex / Cursor / Windsurf / Gemini / Copilot / Cline / Antigravity / Kilo / Augment / Trae…) + manifest 추적 | 도구별 개별 설정 |
